...
export default function TopSells() {
  ...
  return (
    <div
      className={`container mx-auto my-10 px-4 sm:px-6 lg:px-8 py-8 space-y-8 rounded-xl shadow-xl transition-all duration-300 ${
        isDark ? "bg-[#1F2A3C] text-white" : "bg-white text-gray-900"
      }`}
    >
      {/* Header */}
      <h2
        className={`text-2xl font-bold tracking-wide ${
          isDark ? "text-[#2AE89E]" : "text-yellow-600"
        }`}
      >
        TOP CELL PHONES & TABLETS
      </h2>
      ...
      <div className="grid grid-cols-3 gap-6 rounded-lg overflow-hidden">
          ...
          <Slider {...sliderSettings}>
            ...
            <div className="relative flex items-center justify-center rounded-xl shadow-xl overflow-hidden">
              ...
              <div className="absolute left-30 top-1/4 max-w-lg space-y-2">
                ...
                <p className="text-xl">Best Sound Quality</p>
                <p className="mt-2 text-sm">
                  Crystal Clear Bass
                  <br />
                  Immersive Audio
        ...
        {/* Right side promo */}
        <div
          className={`rounded-xl relative shadow-lg flex ${
            isDark ? "bg-[#1F2A3C]" : "bg-gray-100"
          }`}
        >
          <img
            src="/public/ImagesForProducts/div.img.png"
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 text-center">
          {categories.map((cat, idx) => (
            <Link key={idx} to={`/store/${cat.slug}`}>
              <div
                className={`p-4 font-bold text-xs sm:text-sm gap-2 flex flex-col items-center justify-center rounded-lg hover:scale-105 transform transition shadow-md ${
                  isDark
                    ? "bg-[#1A2233] text-white hover:bg-[#2AE89E] hover:text-black"
